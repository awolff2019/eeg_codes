function outvec = aw_powerSliding(data, freq_range, srate)
%
% outvec = aw_powerSliding(data, freq_range, srate)
%
...
for chani=1:size(data,1)
      ...
      %%  compute power sliding
       ...
       if ndims(data) == 3       
    %         outvec(chani,:) = mean(medfilt1(freqslide, filt_order, [], 2,'omitnan'),2);
            outvec(chani,:,:) = medfilt1(freqslide, filt_order, [], 2,'omitnan');
       else
             outvec(chani,:) = medfilt1(freqslide, filt_order, [], 2,'omitnan');
       end
        clear freqslide
